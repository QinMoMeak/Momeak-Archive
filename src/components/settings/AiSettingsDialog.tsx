...
const selectClassName =
  "h-10 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-700 shadow-sm outline-none transition focus:border-slate-300 focus:ring-2 focus:ring-slate-200 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 dark:focus:border-slate-500 dark:focus:ring-slate-700";
        ...
        <div className="border-b border-slate-100 bg-slate-900 px-6 py-5 text-white dark:border-slate-800 dark:bg-slate-950">
          <DialogHeader>
            ...
            <DialogDescription className="mt-2 text-slate-300 dark:text-slate-400">
              仅管理员可见。这里配置的是当前本地仓库运行环境使用的 AI 服务商、模型和 API Key。
            </DialogDescription>
          </DialogHeader>
        </div>

        <div className="space-y-5 p-6 text-slate-700 dark:text-slate-200">
          {settings && (
            <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4 dark:border-slate-800 dark:bg-slate-900/70">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <div className="text-sm text-slate-600 dark:text-slate-300">
                  Provider: {settings.effectiveSettings.provider} · Model: {settings.effectiveSettings.model}
                </div>
              </div>
            ...
            <div className="space-y-2 md:col-span-2">
              ...
              <div className="flex gap-2">
                <Input
                  ...
                  placeholder="留空可保留当前已保存的 Key，或回退到 .env.local 默认值"
                />
                <Button
                  variant="outline"
                  size="icon"
